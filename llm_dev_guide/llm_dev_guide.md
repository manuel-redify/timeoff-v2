...
describe('calculateProRatedAllowance', () => {
  it('should calculate full allowance for start date of Jan 1', () => {
    const result = calculateProRatedAllowance(
      new Date('2026-01-01'),
      25
    );
  ...
  it('should calculate half allowance for start date of Jul 1', () => {
    const result = calculateProRatedAllowance(
      new Date('2026-07-01'),
      24
    );
...
```typescript
// __tests__/api/leave-requests.test.ts
describe('POST /api/leave-requests', () => {
  ...
  it('should reject requests with past dates', async () => {
    // Test validation
  });
});
```
...
```typescript
// e2e/leave-request-flow.spec.ts
...
/>
<p id="start-date-help">Select the first day of your leave</p>
...
3. **Use streaming for slow queries**
   ```typescript
...
```typescript
// Using localStorage (not available in Server Components)
...
const apiKey = process.env.API_KEY; // ⚠️ Exposed to client!

// Not validating user input
const data = await request.json();
await db.insert(data); // ⚠️ No validation!
```
...
import { env } from '@/env.mjs'; // Using t3-env or similar
...
- Check user sync between Clerk and Supabase
- Validate auth middleware is applied
...
2. **Review Legacy Code** → Extract business logic
3. **Design Schema** → Map data models
4. **Implement Backend** → API routes + business logic
5. **Build UI** → Components + pages
6. **Add Tests** → Unit + Integration + E2E
7. **Review Security** → Auth + RLS + Validation
8. **Optimize** → Performance + Accessibility
9. **Document** → Code comments + API docs
10. **Deploy** → Vercel with proper env vars
...
If you encounter:
- **Ambiguous requirements:** Flag in PRD comments
- **Conflicting logic in legacy code:** Document and ask for clarification
- **Missing specifications:** Note what's unclear and propose solution
- **Technical impossibilities:** Explain constraints and suggest alternatives

---

## Resources

- **Legacy Repo:** https://github.com/manuel-redify/timeoff-management-application
...
You've successfully implemented a feature when:
...
*Good luck with development! Remember: When in doubt, refer back to the PRDs and ask for clarification.*

---

*End of LLM Developer Guide*