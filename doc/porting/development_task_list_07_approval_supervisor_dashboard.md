...
### 1. Database & Backend
- [x] **Implement Approval Delegation model**: Add the `ApprovalDelegation` model to `schema.prisma`.
  - **Done looks like**: Prisma schema updated with supervisor/delegate relations and date ranges; migration applied.
- [x] **Pending Approvals Service**: Create a backend service to retrieve all requests requiring action by a specific user (including those delegated to them).
  - **Done looks like**: Service accounts for both direct approval steps and active delegation periods.
- [x] **Bulk Approval API**: Implement `POST /api/approvals/bulk-action` to handle multiple requests in a single transaction.
...
- [x] **Delegation Management Endpoints**: Implement CRUD endpoints for `ApprovalDelegation`.
  - **Done looks like**: Supervisors can manage their own delegations; Admins can manage all.
- [x] **Conflict Detection Logic**: Implement a utility to find overlapping approved leaves within the same department or team.
  - **Done looks like**: Logic correctly flags "busy" periods where too many team members are already off.

### 2. UI & Frontend
- [x] **Supervisor Dashboard View**: Build the specialized approvals dashboard (`/approvals`).
  - **Done looks like**: Interface displays a list of pending requests with employee details, project context, and inline actions.
- [x] **Bulk Action Toolbar**: Add multi-select capabilities and a floating/sticky action bar for bulk operations.
  - **Done looks like**: Supervisors can easily select multiple requests and approve/reject them in one go.
- [x] **Delegation Settings Page**: Build an interface in the supervisor's settings to manage active/scheduled delegations.
  - **Done looks like**: User-friendly form with date pickers and user search for selecting delegates.
- [x] **Conflict Indicator Component**: Add visual markers (tooltips/badges) to request cards when overlaps with other team members are detected.
...
- [x] **Delegation Routing Engine**: Integrate delegation checks into the core approval resolution engine.
  - **Done looks like**: Requests are automatically visible to delegates during the specified timeframe.
- [x] **Allowance Context Integration**: Ensure the dashboard displays the requester's remaining allowance balance.
  - **Done looks like**: Supervisors can see if a requester has sufficient balance before approving.
- [x] **Audit Trail for Approvals**: Ensure all decisions (including bulk and delegated ones) are logged with the correct actor and context.
  - **Done looks like**: Audit logs distinguish between "Direct Approval" and "Approval via Delegate".

## Acceptance Criteria
- [x] Supervisors can see and process all pending requests from their dashboard.
- [x] Bulk approval and rejection (with mandatory comments) are fully functional.
- [x] Delegation correctly transfers approval authority for a defined period.
- [x] Supervisors can see potential leave conflicts and allowance status before deciding.
- [x] All approval data is strictly filtered by company and supervisor permissions via RLS.
